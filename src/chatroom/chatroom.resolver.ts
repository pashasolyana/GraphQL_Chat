...
import * as GrahpQLUpload from 'graphql-upload/GraphQLUpload.js';
...
export class ChatroomResolver {
    ...
    async sendMessage(
        ...
        @Context() context: {req: Request},
        @Args('image', {type: () => GrahpQLUpload, nullable: true})
        image?: GrahpQLUpload,
    ){
        let imagePath = null;
        if(image) imagePath = await this.chatroomService.saveImage(image);
        const newMessage = await this.chatroomService.sendMessages(
            chatroomId,
            content,
    ...
    async getChatroomsForUser(@Args('userId') userId: number){
        return this.chatroomService.getChatroomsForUsers(userId);
    }
